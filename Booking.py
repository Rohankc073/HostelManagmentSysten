...
def Database():
    ...
    cursor = conn.cursor()
    #creating ROOM_Booking  table
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS ROOM_BOOKING (STU_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, STU_NAME TEXT, STU_CONTACT TEXT, STU_EMAIL TEXT, STU_PARENTS_NAME TEXT, STU_ADDRESS TEXT)")
...
def DisplayForm():
    ...
    display_screen.title("BOOKING SYSTEM")
    ...
    #label for heading
    lbl_text = Label(TopViewForm, text="Room Booking Of Hostel Management System", font=('Calisto MT Bold', 18), width=600,bg="#06518d",fg="white")
    ...
    Button(LFrom,text="Book",bg='#06518d',font=("Calisto MT Bold", 10),command=register,fg='white').pack(side=TOP, padx=10,pady=5, fill=X)

    def back():
        tkMessageBox.showinfo('Message', 'Do you really want to go back?')
        display_screen.destroy()
        import dashboard
        import dashboard

    Button(LFrom, text="Back", bg='#06518d', font=("Calisto MT Bold", 10), fg='white', command=back).pack(side=TOP,
                                                                                                          padx=10,
                                                                                                          pady=5,
                                                                                                          fill=X)
    ...
    #Edit button
    edit_delete = Button(LeftViewForm, text="Edit", bg='#06518d',command=Edit,fg='white',font=("Calisto MT Bold", 12))
    edit_delete.pack(side=TOP, padx=10, pady=10, fill=X)
...
def register():
    ...
    if name1=='' or con1==''or email1=='' or room1==''or address1=='':
        ...
        #execute query
        conn.execute('INSERT INTO ROOM_BOOKING (STU_NAME , STU_CONTACT , STU_EMAIL , STU_PARENTS_NAME , STU_ADDRESS ) \
              VALUES (?,?,?,?,?)',(name1,con1,email1,room1,address1));
        conn.commit()
        tkMessageBox.showinfo("Message","Booked successfully")
        #refresh table data
        DisplayData()
        name.set("")
        contact.set("")
...
def Delete():
    ...
    if not tree.selection():
        ...
        if result == 'yes':
            ...
            tree.delete(curItem)
            cursor=conn.execute("DELETE FROM ROOM_BOOKING WHERE STU_ID = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
...
def SearchRecord():
    ...
    if SEARCH.get() != "":
        ...
        cursor=conn.execute("SELECT * FROM ROOM_BOOKING WHERE STU_NAME LIKE ?", ('%' + str(SEARCH.get()) + '%',))
...
def DisplayData():
    ...
    tree.delete(*tree.get_children())
    #select query
    cursor=conn.execute("SELECT * FROM ROOM_BOOKING")
...
def Edit():
    ...
    if name1 == '' or contact1 == '' or email1 == '' or roomno1 == '' or address1 == '':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE ROOM_BOOKING SET STU_NAME=?, STU_CONTACT=?, STU_EMAIL=?, STU_PARENTS_NAME=?, STU_ADDRESS=? WHERE STU_ID=?",
            (name1, contact1, email1, roomno1, address1, id))
        conn.commit()
        conn.close()
        DisplayData()
        name.set("")
